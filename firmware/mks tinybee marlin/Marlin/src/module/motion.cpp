...
void prepare_line_to_destination() {
  apply_motion_limits(destination);
    ...
    if (!DEBUGGING(DRYRUN) && destination.e != current_position.e) {
      // Разрешить движение E, если нагрев выключен (целевая температура 0)
      bool ignore_e = (thermalManager.degTargetHotend(active_extruder) != 0) && thermalManager.tooColdToExtrude(active_extruder);
