  ...
  millis_t MarlinUI::next_button_update_ms; // = 0
    ...
    bool MarlinUI::handle_keypad() {
        ...
        if (keypad_buttons) {
            ...
            #if HAS_MARLINUI_MENU
              // REVERSE_MENU_DIRECTION is forced by ZONESTAR_LCD in Conditionals_LCD.h.
              // This reverses encoderDirection for menu scrolling. draw_edit_screen()
              // calls encoder_direction_normal() which resets it to normal.
              // Check encoderDirection to determine if UP/DOWN need swapping:
              //   - Menu scrolling (reversed): UP=decrement, DOWN=increment
              //   - Value editing (normal):    UP=increment, DOWN=decrement
