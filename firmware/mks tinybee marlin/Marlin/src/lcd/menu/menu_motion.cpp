...
void _menu_move_distance(const AxisEnum axis, const screenFunc_t func, const int8_t eindex=active_extruder) {
  ...
  inline void _menu_move_distance_e_maybe() {
    // Не показывать диалог, если нагрев экструдера выключен (целевая температура 0)
    if (thermalManager.tooColdToExtrude(active_extruder) && (thermalManager.degTargetHotend(active_extruder) != 0)) {
      ui.goto_screen([]{
